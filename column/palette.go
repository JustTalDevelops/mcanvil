...
// NewFilledListPalette returns a new filled list palette.
func NewFilledListPalette(bitsPerEntry int32, data []int32) *ListPalette {
	maxId := int32((1 << bitsPerEntry) - 1)
	return &ListPalette{
		data:   data,
		maxId:  maxId,
		nextId: int32(len(data)),
	}
}
...
// NewFilledMapPalette returns a new filled map palette.
func NewFilledMapPalette(bitsPerEntry int32, data []int32) *MapPalette {
	maxId := int32((1 << bitsPerEntry) - 1)
	idToState := make([]int32, maxId+1)
	stateToID := make(map[int32]int32)
	for i, state := range data {
		idToState[i] = state
		stateToID[state] = int32(i)
	}
	return &MapPalette{
		maxId:     maxId,
		idToState: idToState,
